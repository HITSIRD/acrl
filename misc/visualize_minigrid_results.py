...
def precision_comparison(ax=None, path=None, base_log_dir="logs"):
    if ax is None:
        ...
        ax = plt.Axes(f, [0.17, 0.17, 0.76, 0.7])
        f.add_axes(ax)
        show = True
    else:
        f = plt.gcf()
...
def performance_plot(ax=None, path=None, base_log_dir="logs"):
    ...
    for method, color in zip(["self_paced", "random", "default", "wasserstein", "goal_gan", "alp_gmm",
        ...
        exp = MinigridAExperiment(base_log_dir, method, "ppo", {}, seed=1)
        log_dir = os.path.dirname(exp.get_log_dir())
    ...
    if show:
        ax.legend(lines,
                 [r"\sprl", "Random", "Oracle", r"\currot", r"\goalgan", r"\alpgmm", r"\acl", r"\plr", r"\vds", "acrl (ours)"],
...
if __name__ == "__main__":
    ...
    performance_plot(path="figures/minigrid_a_performance.pdf", base_log_dir=base_log_dir)
