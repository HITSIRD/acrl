...
from deep_sprl.environments.minigrid.core.constants import OBJECT_TO_IDX, TILE_PIXELS
from deep_sprl.environments.minigrid.core.world_object import Wall, WorldObj
from deep_sprl.environments.minigrid.utils.rendering import (
    downsample,
    fill_coords,
    highlight_img,
    point_in_rect,
...
class Grid:
    ...
    def encode(self, vis_mask: np.ndarray | None = None, agent_pos: tuple[int, int] | None = None,
               agent_dir: int | None = None) -> np.ndarray:
        """
                ...
                if vis_mask[i, j]:
                    ...
                    if agent_pos is not None and agent_dir is not None:
                        if i == agent_pos[0] and j == agent_pos[1]:
                            ...
                            array[i, j, 1] = 0
                            array[i, j, 2] = agent_dir

        return array

    def encode_all(self, agent_pos: tuple[int, int] | None = None, agent_dir: int | None = None) -> np.ndarray:
        """
        Produce a compact numpy encoding of the grid
        """
