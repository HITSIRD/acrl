...
class CurriculumType(Enum):
    ...
    def acrl(self):
        return self.value == CurriculumType.ACRL.value
    ...
    def from_string(string):
        ...
        elif string == str(CurriculumType.ACRL):
            return CurriculumType.ACRL
        else:
...
class PPOInterface(AgentInterface):
    ...
    def mean_policy_std(self, cb_args, cb_kwargs):
        # std_th = self.learner.policy.get_distribution(torch.zeros((1, self.obs_dim)))
        # std_th = std_th.distribution.stddev[0, :]
        # std_th = std_th.distribution.stddev[0]
...
class ExperimentCallback:
    ...
    def __init__(self, log_directory, learner, env_wrapper, save_interval=5, step_divider=1):
        ...
        self.iteration = 0
        self.last_time = None
        self.format = "   %4d    | %.1E |   %3d    |  %.2E  |  %.2E  |  %.2E  | %d"
        ...
        header = " Iteration |  Time   | Ep. Len. | Mean Reward | Mean Disc. Reward | Mean Policy STD | Step"
        if self.env_wrapper.teacher is not None:
            if isinstance(self.env_wrapper.teacher, SelfPacedTeacherV2):
    ...
    def __call__(self, *args, **kwargs):
        if self.algorithm_iteration % self.step_divider == 0:
            ...
            mean_rew, mean_disc_rew, mean_length, steps, _ = self.env_wrapper.get_statistics()
...
class AbstractExperiment(ABC):
    APPENDIX_KEYS = {"default": ["DISCOUNT_FACTOR", "STEPS_PER_ITER", "LAM"],
                     ...
                     CurriculumType.ACRL: ["ACRL_LSP_RATIO", "ACRL_EBU_RATIO"]}
    ...
    def process_parameters(self):
        allowed_overrides = {"DISCOUNT_FACTOR": float, "MAX_KL": float, "STEPS_PER_ITER": int,
                             ...
                             "ACRL_LSP_RATIO": float, "ACRL_EBU_RATIO": float}
